...
#creates the player's pokemon, using presets for each contest category read from a csv file
def initPlayerMon(gameMode, category):  
    player = None
    
    #define which csv file to read from based on our game mode
...
#creates NPC opponents by reading data from a csv file, with presets for each contest category
...
def main():
    ...
    with open('init.txt', 'r') as txtfile:
        gameMode = txtfile.readline().strip() #1st line of init.txt is the game mode
        myCategory = txtfile.readline().strip().lower() #2nd line of init.txt is the contest category
    ...
    #initialize the contestants based on input csv files
    #player
    ...
    for i in range (0, 5):
        ...
        #prompt the user to input their move choice
        ...
        #after the player selects a move, each pokemon contestant performs a move in the order decided for this round
        for i in range(0, 4):
            if myStage.getMonsInTurnOrder()[i] is mon1:
                ...
                myStage.getMonsInTurnOrder()[i].doMove(random.randint(0,3), i, myStage.getMonsInTurnOrder(), myStage.excitementLevel) #NPC performs a random move from their movepool
            myStage.updateExcitementLevel(myStage.getMonsInTurnOrder()[i], i) #update audience excitement level after a move is performed
            print()
